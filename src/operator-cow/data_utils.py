...
def get_dataset(args):
    ...
    args.train_num = (
        int(args.train_num) if args.train_num not in ["all", "none"] else args.train_num
    )
    args.test_num = (
        int(args.test_num) if args.test_num not in ["all", "none"] else args.test_num
    )
...
def get_model(args):
    ...
    trunk_size, theta_size, branch_sizes, output_size = (
        args.dataset_config["input_dim"],
        args.dataset_config["theta_dim"],
        args.dataset_config["branch_sizes"],
        args.dataset_config["output_dim"],
    )
    output_size = (
        args.dataset_config["output_dim"]
        if args.component in ["all", "all-reduce"]
        else 1
    )
...
def get_loss_func(name, args, **kwargs):
    if name == "rel2":
        return WeightedLpRelLoss(
            p=2, component=args.component, normalizer=kwargs["normalizer"]
        )
    elif name == "rel1":
        return WeightedLpRelLoss(
            p=1, component=args.component, normalizer=kwargs["normalizer"]
        )
    elif name == "l2":
        return WeightedLpLoss(
            p=2, component=args.component, normalizer=kwargs["normalizer"]
        )
    elif name == "l1":
        return WeightedLpLoss(
...
class FNODataset(DGLDataset):
    ...
    def __normalize_y(self):
        ...
        for g in self.graphs:
            g.ndata["y"] = self.y_normalizer.transform(
                g.ndata["y"], inverse=False
            )  # a torch quantile transformer
    ...
    for sample in transposed:
        if isinstance(sample[0], dgl.DGLGraph):
            ...
            sample_ = MultipleTensors(
                [
                    pad_sequence([sample[i][j] for i in range(len(sample))]).permute(
                        1, 0, 2
                    )
...
class MIODataset(DGLDataset):
    def __init__(
        ...
        self.cached_path = (
            self.data_path[:-4] + "_" + "train" + "_cached" + self.data_path[-4:]
            if train
            else self.data_path[:-4] + "_" + "test" + "_cached" + self.data_path[-4:]
        )
            ...
            if self.train:
                if train_num == "all":  # use all to train
                    ...
                    if train_num > len(data_all):
                        print(
                            "Warnings: there is no enough train data {} / {}".format(
                                train_num, len(data_all)
                            )
                        )
                ...
                if test_num == "all":
                    ...
                    if test_num > len(data_all):
                        print(
                            "Warnings: there is no enough test data {} / {}".format(
                                test_num, len(data_all)
                            )
                        )
    ...
    def process(self):
        ...
        for i in range(len(self)):
            ...
            if input_f is not None:
                input_f = MultipleTensors(
                    [torch.from_numpy(f).float() for f in input_f]
                )
                self.inputs_f.append(input_f)
                self.num_inputs = len(input_f)

        if len(self.inputs_f) == 0:
            self.inputs_f = torch.zeros(
                [len(self)]
            )  # pad values, tensor of 0, not list
    ...
    def __sort_dataset(self):
        ...
        sorted_lists = sorted(
            zipped_lists, key=lambda x: x[0].number_of_nodes(), reverse=True
        )
    ...
    def __normalize_y(self):
        if self.y_normalizer is None:
            ...
            #    print(self.y_normalizer.max, self.y_normalizer.min)
                ...
                self.y_normalizer = TorchQuantileTransformer(
                    self.y_normalizer.output_distribution,
                    self.y_normalizer.references_,
                    self.y_normalizer.quantiles_,
                )
    ...
    def __update_dataset_config(self):
        self.config = {
            ...
            "branch_sizes": (
                [x.shape[1] for x in self.inputs_f[0]]
                if isinstance(self.inputs_f, list)
                else 0
            ),
        }
        return
...
class MIODataLoader(torch.utils.data.DataLoader):
    def __init__(
        ...
        super(MIODataLoader, self).__init__(
            dataset=dataset,
        ...
        if sort_data:
            ...
            self.batch_indices = (
                list(range(0, (len(dataset) // batch_size) * batch_size))
                if drop_last
                else list(range(0, len(dataset)))
            )
        if shuffle:
...
class WeightedLpRelLoss(_WeightedLoss):
    def __init__(self, d=2, p=2, component=0, regularizer=False, normalizer=None):
        ...
        self.p = p
        self.component = (
            component if component == "all" or "all-reduce" else int(component)
        )
    ...
    def _lp_losses(self, g, pred, target):
        if (self.component == "all") or (self.component == "all-reduce"):
            ...
            err_pool = self.sum_pool(
                g, (pred - target[:, self.component]).abs() ** self.p
            )
    ...
    def forward(self, g, pred, target):
        ...
        if self.normalizer is not None:
            ori_pred, ori_target = self.normalizer.transform(
                pred, component=self.component, inverse=True
