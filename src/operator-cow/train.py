...
from data_utils import MIODataLoader, collate_op, get_dataset, get_loss_func, get_model
from models.optimizer import Adam, AdamW
from torch.optim.lr_scheduler import LambdaLR, OneCycleLR, StepLR
from torch.utils.tensorboard import SummaryWriter
from utils import get_num_params, get_seed
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch in train_loader:
            ...
            if writer is not None:
                for j in range(len(_loss_mean)):
                    writer.add_scalar(
                        "train_loss_{}".format(j), _loss_mean[j], it
                    )  #### loss 0 seems to be the sum of all loss
        ...
        if writer is not None:
            if val_result["metric"].size == 1:
                writer.add_scalar(
                    "val loss {}".format(metric_func.component), val_metric, epoch
                )
            else:
                for i, metric_i in enumerate(val_result["metric"]):
                    writer.add_scalar("val loss {}".format(i), metric_i, epoch)

        log += "| best val: {:.6f} at epoch {} | current lr: {:.3e}".format(
            best_val_metric, best_val_epoch + 1, lr
        )
...
def train_batch(
    model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.999
):
    optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    args.normalizer = (
        ...
        if train_dataset.y_normalizer is not None
        else None
    )
    ...
    loss_func = get_loss_func(
        name=args.loss_name, args=args, regularizer=True, normalizer=args.normalizer
    )
    metric_func = get_loss_func(
        name="rel2", args=args, regularizer=False, normalizer=args.normalizer
    )
    ...
    path_prefix = (
        args.dataset
        + "_{}_".format(args.component)
        + model.__name__
        + args.comment
        + time.strftime("_%m%d_%H_%M_%S")
    )
    ...
    if args.lr_method == "cycle":
        ...
        scheduler = StepLR(
            optimizer, step_size=args.lr_step_size * len(train_loader), gamma=0.7
        )
        ...
        scheduler = LambdaLR(
            ...
            lambda steps: min(
                ...
                np.power(
                    args.warmup_epochs * len(train_loader) / float(steps + 1), 0.5
                ),
            ),
        )
