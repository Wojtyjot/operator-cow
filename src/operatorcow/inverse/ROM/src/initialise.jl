...
function buildVessel(ID :: Int, vessel_data :: Dict{Any,Any}, blood :: Blood, jump :: Int)
    ...
    Q_t = zeros(Float64, jump, M+1)
    P_t = zeros(Float64, jump, M+1)
    A_t = zeros(Float64, jump, M+1)
    u_t = zeros(Float64, jump, M+1)
    c_t = zeros(Float64, jump, M+1)
    Q_l = zeros(Float64, jump, M+1)
    P_l = zeros(Float64, jump, M+1)
    A_l = zeros(Float64, jump, M+1)
    u_l = zeros(Float64, jump, M+1)
    c_l = zeros(Float64, jump, M+1)
