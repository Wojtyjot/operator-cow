...
def horizontal_fourier_embedding(X, n=3):
    ...
    X = torch.cat([X.unsqueeze(-1), X_cos, X_sin], dim=-1).view(
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x, y=None, layer_past=None):
        ...
        q = (
            self.query(x).view(B, T1, self.n_head, C // self.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
        k = (
            self.key(y).view(B, T2, self.n_head, C // self.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
        v = (
            self.value(y).view(B, T2, self.n_head, C // self.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
...
class LinearCrossAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.keys = nn.ModuleList(
            [nn.Linear(config.n_embd, config.n_embd) for _ in range(config.n_inputs)]
        )
    ...
    def forward(self, x, y=None, layer_past=None):
        ...
        for i in range(self.n_inputs):
            ...
            k = (
                self.keys[i](y[i])
                .view(B, T2, self.n_head, C // self.n_head)
                .transpose(1, 2)
            )  # (B, nh, T, hs)
            v = (
                self.values[i](y[i])
...
class CGPTNO(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[
                CrossAttentionBlock(self.gpt_config)
    ...
    def configure_optimizers(
        self, lr=1e-3, betas=(0.9, 0.999), weight_decay=0.00001, no_decay_extras=[]
    ):
        """
