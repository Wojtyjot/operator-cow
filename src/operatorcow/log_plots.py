...
def plot_pred_paper(
    ...
    axs[0, 0].plot(
        unit_to_mmHg(ground_truth[:100, 0]),
        label="Ground truth",
        color="black",
        linewidth=1,
    )
    axs[0, 0].plot(
        unit_to_mmHg(predictions[:100, 0]),
        label="Predicted",
        color="red",
        linestyle="--",
        linewidth=1,
    ...
    axs[0, 1].plot(
        ground_truth[:100, 1], label="Ground truth", color="black", linewidth=1
    )
    axs[0, 1].plot(
        predictions[:100, 1],
        label="Predicted",
    ...
    axs[0, 1].grid()
    axs[1, 0].plot(
        ground_truth[:100, 2], label="Ground truth", color="black", linewidth=1
    )
    axs[1, 0].plot(
        predictions[:100, 2],
        label="Predicted",
    ...
    plt.setp(
        axs[1, 0].get_yticklabels(),
        fontsize=7,
    )
    axs[1, 0].legend(fontsize=5)
    axs[1, 0].grid()
    axs[1, 1].plot(
        ground_truth[:100, 2] * ground_truth[:100, 1],
        label="Ground truth",
        color="black",
        linewidth=1,
    )
    axs[1, 1].plot(
        predictions[:100, 2] * predictions[:100, 1],
        label="Predicted",
        color="red",
        linestyle="--",
        linewidth=1,
...
def VANO_paper_full(
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            z_samples = mean.unsqueeze(0) + torch.exp(log_var / 2).unsqueeze(
                0
            ) * torch.rand(z.shape, device=z.device)
    ...
    for idx, artery in enumerate(arteries):
        ...
        for i in range(len(results[artery]["true"]) - 1):
            ...
            # sys.exit()
            axs_1[idx].plot(
                results[artery]["true"][i][0].cpu().numpy(),
                color="gray",
                alpha=0.2,
                linewidth=0.7,
            )
            axs_2[idx].plot(
                results[artery]["pred"][i][0].cpu().numpy(),
                color="gray",
                alpha=0.2,
                linewidth=0.7,
            )
        ...
        axs_1[idx].plot(
            results[artery]["true"][-1][0].cpu().numpy(),
            color="black",
            alpha=1,
            linewidth=0.7,
        )
        ...
        axs_1[idx].grid()
        plt.setp(
            axs_1[idx].get_xticklabels(),
            fontsize=7,
        )
        plt.setp(
            axs_1[idx].get_yticklabels(),
            fontsize=7,
        )

        axs_2[idx].plot(
            results[artery]["pred"][-1][0].cpu().numpy(),
            color="black",
            alpha=1,
            linewidth=0.7,
        )
        ...
        axs_2[idx].grid()
        plt.setp(
            axs_2[idx].get_xticklabels(),
            fontsize=7,
        )
        plt.setp(
            axs_2[idx].get_yticklabels(),
            fontsize=7,
        )
    fig_1.tight_layout()
    fig_2.tight_layout()
