runai submit \
  ...
  -e PROJECT_ROOT_AT=/claire-rcp-scratch/home/moalla/operator-cow/run \
  -- python -m operator-cow.template_experiment some_arg=2

# template_experiment is an actual script that you can run.
# or -- zsh operator-cow/reproducibility-scripts/template-experiment.sh
...
# That would be a copy of the operator-cow repo frozen in a commit at a working state to be used in unattended jobs.
