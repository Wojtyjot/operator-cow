...
PROJECT_ROOT_AT=/project/Operator-cow
ACCELERATION=cpu
WANDB_API_KEY=
...
PROJECT_NAME=Operator-cow
PACKAGE_NAME=Operator-cow
...
push_usr_or_root() {
  ...
  if [ "${PUSH_IMAGE_NAME}" == "" ]; then
    ...
    echo "For example: ./template.sh push docker.io/skandermoalla/Operator-cow"
